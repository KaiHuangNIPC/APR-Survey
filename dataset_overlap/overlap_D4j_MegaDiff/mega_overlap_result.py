...
def overlap_set():
    ...
    id_dict = {}
    for id in D4j_ID_list:
        id_dict[id.split('-')[0]] = []
    for id in D4j_ID_list:
        id_dict[id.split('-')[0]].append(id)
        
    # print(id_dict)
    for id_index in id_dict:
        print(id_index, len(id_dict[id_index]))
        # print(id_dict[id_index])
...
def overlap_result(input_line):
    with open('/SS970evo/datasets/dataset_overlap/overlap_D4j_MegaDiff/overlap_filename_info.txt', 'r') as f2:
    ...
    BFP_list = D4j_BFPs[D4j_project.split(':')[1]]
    D4j_id = BFP_list[0]
    D4j_bugs = BFP_list[5]
    D4j_fixs = BFP_list[6]
    D4j_rems = BFP_list[7]
